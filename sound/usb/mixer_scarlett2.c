...
enum {
	SCARLETT2_AUTOGAIN_STATUS_STOPPED   = 0,
	SCARLETT2_AUTOGAIN_STATUS_RUNNING   = 1,
	SCARLETT2_AUTOGAIN_STATUS_FAILED    = 2,
	SCARLETT2_AUTOGAIN_STATUS_CANCELLED = 3,
	SCARLETT2_AUTOGAIN_STATUS_UNKNOWN   = 4,
	SCARLETT2_AUTOGAIN_STATUS_COUNT     = 5,
};

/* Power Status Values */
enum {
	SCARLETT2_POWER_STATUS_EXT   = 0,
	SCARLETT2_POWER_STATUS_BUS   = 1,
	SCARLETT2_POWER_STATUS_FAIL  = 2,
	SCARLETT2_POWER_STATUS_COUNT = 3,
};
...
static void scarlett2_notify_input_autogain(struct usb_mixer_interface *mixer);
...
static const struct scarlett2_notification scarlett4_2i2_notifications[] = {
	...
	{ 0x00400000, scarlett2_notify_input_autogain },
...
static const struct scarlett2_notification scarlett4_4i4_notifications[] = {
	...
	{ 0x40000000, scarlett2_notify_power_status }, /* power fail */
 ...
 * current/next state
 */
static void scarlett2_decode_muteable(uint8_t *v)
{
	*v = (*v ^ (*v >> 1)) & 1;
}
...
static int scarlett2_update_input_level(struct usb_mixer_interface *mixer)
{
	struct scarlett2_data *private = mixer->private_data;
	const struct scarlett2_device_info *info = private->info;
	int count = info->level_input_count + info->level_input_first;
	int err, i;
	...
	err = scarlett2_usb_get_config(
		...
		count, private->level_switch);
	if (err < 0)
		return err;
	if (private->config_set->items[SCARLETT2_CONFIG_LEVEL_SWITCH].mute)
		for (i = 0; i < count; i++)
			scarlett2_decode_muteable(&private->level_switch[i]);
					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	/* To set the Gen 4 mutable controls, bit 1 gets set */
	if (private->config_set->items[SCARLETT2_CONFIG_LEVEL_SWITCH].mute)
		val = val ? 0x02 : 0x03;
...
static int scarlett2_update_input_phantom(struct usb_mixer_interface *mixer)
{
	...
	if (private->config_set->items[SCARLETT2_CONFIG_PHANTOM_SWITCH].mute)
		for (i = 0; i < info->phantom_count; i++)
			scarlett2_decode_muteable(&private->phantom_switch[i]);
...
static int scarlett2_update_autogain(struct usb_mixer_interface *mixer)
{
	...
	private->input_autogain_updated = 0;
	...
	struct snd_kcontrol *kctl, struct snd_ctl_elem_value *ucontrol)
{
	...
	if (private->input_autogain_updated) {
...
static int scarlett2_add_line_in_ctls(struct usb_mixer_interface *mixer)
{
	...
	if (info->inputs_per_phantom == 1) {
		for (i = 0; i < info->phantom_count; i++) {
			snprintf(s, sizeof(s), fmt, i + 1 + info->phantom_first,
...
static int scarlett2_update_power_status(struct usb_mixer_interface *mixer)
{
	...
	err = scarlett2_usb_get_config(mixer, SCARLETT2_CONFIG_POWER_FAIL,
				       1, &power_fail);
	if (err < 0)
		return err;
...
static int scarlett2_add_power_status_ctl(struct usb_mixer_interface *mixer)
{
	...
	return scarlett2_add_new_ctl(mixer, &scarlett2_power_status_ctl,
				     0, 1, "Power Status Card Enum", NULL);
}
...
static int scarlett2_read_configs(struct usb_mixer_interface *mixer)
{
	...
	if (private->firmware_version < info->min_firmware_version) {
		usb_audio_err(mixer->chip,
			      ...
			      private->firmware_version);
		usb_audio_info(mixer->chip,
			       "Get the scarlett2 firmware update utility from "
...
static void scarlett2_notify_input_autogain(struct usb_mixer_interface *mixer)
{
	...
	private->input_autogain_updated = 1;
