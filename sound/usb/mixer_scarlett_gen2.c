...
/* Interrupt flags from the interface */
				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] = private->sync;
	return 0;
}
...
static void scarlett2_count_mux_io(const struct scarlett2_ports *ports,
				   struct scarlett2_mixer_data *private)
{
				       ...
				       struct scarlett2_mixer_data *private)
{
	...
	for (i = 0; i < config->desc.bNumInterfaces; i++) {
		...
		private->wMaxPacketSize = epd->wMaxPacketSize;
				  ...
				  const struct scarlett2_device_info *info)
{
	...
	private->info = info;
	scarlett2_count_mux_io(info->ports, private);
...
static void scarlett2_mixer_interrupt_sync_change(
	struct usb_mixer_interface *mixer)
{
	struct scarlett2_mixer_data *private = mixer->private_data;
...
static void scarlett2_mixer_interrupt(struct urb *urb)
{
	...
	if (len == 8) {
		...
		if (data & SCARLETT2_USB_INTERRUPT_SYNC_CHANGE)
			scarlett2_mixer_interrupt_sync_change(mixer);
